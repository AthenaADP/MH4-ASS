...
namespace MH4ASS {
	...
	public:
		SkillHelp( const int static_index, const bool skill_based, const bool alphabetic )
		{
			InitializeComponent();

			txtDescription->Text = StaticString( Description );
			
			rdoSkills->Text = StaticString( Skills );
...
#pragma endregion
		...
		System::Void SelectInfoInternal( String^ name )
		{
			...
			else
			{
				...
				if( ability == nullptr )
				{
					if( name == Ability::torso_inc->name )
						ability = Ability::torso_inc;
					else return;
				}
			...
			if( last_selected_ability == Ability::torso_inc )
			{
				txtDescription->Text = Ability::torso_inc_desc;
				txtDescription->AppendText( Environment::NewLine );
			}
			else
			{
		...
		System::Void cmsDescription_Opening(System::Object^  sender, System::ComponentModel::CancelEventArgs^  e)
		{
			...
			{
				Utility::UpdateContextMenu( cmsDescription, armors[ line - armor_list_begin ] );
				e->Cancel = false;
			}
		}
};
