...
public:
		...
		{
			//type,slots[,skill1,pt1][,skill2,pt2]
			array< String^ >^ lines = IO::File::ReadAllLines( filename, System::Text::Encoding::GetEncoding( L"shift-jis" ) );
			unsigned i = 0;
			for each( String^% line in lines )
			{
				...
				charm->custom = true;
				charm->num_slots = Convert::ToUInt32( split[ 1 ] );
				if( split.Count >= 4 && split[ 3 ] != L"" )
				{
					{
						Ability^ ab = Ability::FindCharmAbility( split[ 2 ] );
						int pt;
						if( !ConvertInt( pt, split[ 3 ], StringTable::StringIndex::MasaxMHCharmListCorrupted ) )
							return;
						
						charm->abilities.Add( gcnew AbilityPair( ab, pt ) );
					}
					if( split.Count == 6 && split[ 5 ] != L""  )
					{
						Ability^ ab = Ability::FindCharmAbility( split[ 4 ] );
						int pt;
						if( !ConvertInt( pt, split[ 5 ], StringTable::StringIndex::MasaxMHCharmListCorrupted ) )
							return;

						charm->abilities.Add( gcnew AbilityPair( ab, pt ) );
					}
				}
				/*if( global_charms.Count > 0 )
				{
					bool found = false;
					for each( Charm^% c in global_charms )
					{
						if( c == charm || c->StrictlyBetterThan( charm ) )
						{
							found = true;
							break;
						}
		...
		System::Void btnSort_Click(System::Object^  sender, System::EventArgs^  e)
		{
			...
			charms.Sort( gcnew Comparison< Charm^ >( CompareCharms ) );
			RefreshCharmList();
		}
};
}
