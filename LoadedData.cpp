...
void LoadedData::LoadDataFiles()
{
	...
	/*DumpNames( Armor::static_armors[0], L"Data/Languages/Japanese/head.txt" );
	DumpNames( Armor::static_armors[1], L"Data/Languages/Japanese/body.txt" );
	DumpNames( Armor::static_armors[2], L"Data/Languages/Japanese/arms.txt" );
	DumpNames( Armor::static_armors[3], L"Data/Languages/Japanese/waist.txt" );
	DumpNames( Armor::static_armors[4], L"Data/Languages/Japanese/legs.txt" );
	DumpNames( %Decoration::static_decorations, L"Data/Languages/Japanese/decorations.txt" );
	DumpSkills( L"Data/Languages/Japanese/skills.txt" );*/

#ifdef CREATE_MATERIALS
	DumpNames( %Material::static_materials, L"Data/components.txt" );
#endif
...
void LoadedData::GetRelevantData( Query^ query )
{
	...
	for( int i = 0; i < int( Armor::ArmorType::NumArmorTypes ); ++i )
	{
		...
		if( ExcavatedGear::armor && query->allow_excavated )
			GetRelevantArmors( query, query->rel_armor[ i ], ExcavatedGear::armor[ i ], query->inf_armor[ i ], %danger_skills );
		query->rel_armor[ i ]->TrimExcess();
	}
	query->rel_armor.TrimExcess();

	//get relevant weapons
	if( query->weapon_slots_allowed == 0 && query->allow_excavated )
	{
